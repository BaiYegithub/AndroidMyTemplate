 ...
 */
    ...
    /**
     * 添加单个数据,不触发刷新 比较耗费资源
     * 如果想刷新 添加数据完毕后请调用 notifyDataChanged
     *
     * @param item item
     */
    public void add(T item,boolean isRefresh) {
        dataSet.data.add(item);
        if(isRefresh) {
            dataSet.notifyContent();
            notifyDataSetChanged(); //调用就会刷新界面，比较耗费资源，
        }
    }
     ...
     * @param data list of data
     */
    public void addAll(List<? extends T> data,boolean isRefresh) {
    ...
    public void insert(int adapterPosition, T item) {
        insert(adapterPosition, item,true);
    }
     ...
     * @param item            插入的数据
     */
    public void insert(int adapterPosition, T item,boolean isRefresh) {
    ...
    /**
     * 直接将数据插入到Data中(不包含头条和尾)
     *
     * @param adapterPosition 插入的位置
     * @param item            插入的数据
     */
    public void insertData(int adapterPosition, T item,boolean isRefresh) {
        dataSet.data.insertImpl(adapterPosition, item);
        if(isRefresh)
            notifyItemInserted(adapterPosition);
    }
