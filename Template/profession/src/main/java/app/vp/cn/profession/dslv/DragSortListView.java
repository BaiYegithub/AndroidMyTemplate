...
package app.vp.cn.profession.dslv;
...
public class DragSortListView extends ListView {
    ...
    //添加不参加滑动改变位置的position
    public void addNoChangeItemPosition(int position) {

        try {
            //得到私有字段
            Field privateMHeaderViewInfos = ListView.class.getDeclaredField("mHeaderViewInfos");
            //通过反射设置私有对象可以访问
            privateMHeaderViewInfos.setAccessible(true);
            ArrayList<FixedViewInfo> mHeaderViewInfos = (ArrayList<FixedViewInfo>) privateMHeaderViewInfos.get(this);
            FixedViewInfo fixedViewInfo = new FixedViewInfo();
            fixedViewInfo.view = getChildAt(position);
            mHeaderViewInfos.add(position,fixedViewInfo);
        } catch (Exception e) {
            e.printStackTrace();
        }


       /* if (noChangeItemsSets == null) {
            noChangeItemsSets = new ArraySet<>();
        }
        noChangeItemsSets.add(position);*/
    }

    //移除不参加滑动位置改变的position 业务逻辑
    public void removeNoChangeItemPosition(int position) {
        ...
        /*if (noChangeItemsSets != null && noChangeItemsSets.size() > position) {
            noChangeItemsSets.remove(position);
        }*/
    }
